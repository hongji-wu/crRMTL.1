#' @name RMTLd_test
#' @title Comparing the difference of restricted mean time lost (RMTLd) under competing risks
#'
#' @description Performs two-sample comparisons using the difference of restricted mean time
...
#' @importFrom crskdiag diag_crr Crsk
#'
...
#' and event 2, and corresponding variance and CI in group 0 and 1.}
#' @return \item{Test.event1}{
#' Three kinds of effect size between groups of event 1, including
#' Cause-specific Hazard Ratios(cHR),Subdistributional Hazard Ratio(sHR)
#' and RMTLd, and corresponding confidence intervals.
#' Meanwhile, the Z-statistics and P-value are estimated via log-rank test,
#' Gray's test and RMTLd test.}
#' @return \item{Test.event2}{
#' Three kinds of effect size between groups of event 2, including
#' Cause-specific Hazard Ratios(cHR),Subdistributional Hazard Ratio(sHR)
#' RMTLd, and corresponding confidence intervals.
#' Meanwhile, the Z-statistics and P-value are estimated via log-rank test,
#' Gray's test and RMTLd test.}
#'
#' @export
...
#' library(crRMTL)
#' data(Example1)
#' # The time point is set as the minimum of the largest follow-up time of two groups when tau = NULL.
#' RMTLd_test(Example1$time, Example1$status, Example1$group, alpha = 0.05, digits = 3, tau = NULL)
#'
#' # The time point can be specified as fixed timepoint,like tau = 10.
...
RMTLd_test <- function(time, status, group, alpha = 0.05, digits = 3, tau = NULL){
  ...
  out2 <- matrix(0,1,2)
  cox1 <- cox.zph(coxph(Surv(time,(status==1)*1)~group))$table[1,3]
  fgm1 <- diag_crr(Crsk(time,status)~group,test="prop",seed=2022)$pval[1]
  out2 <- c(cox1, fgm1)
